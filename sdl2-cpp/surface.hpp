...
namespace sdl2::surface
{
	...
	public:
		...
		[[nodiscard]] Uint32 get_color_key() const
		{
			Uint32 key;
			if (SDL_GetColorKey(ptr, &key))
			{
				return key;
			}
			else
		...
		void set_color_key(Uint32 key, int flag = SDL_TRUE)
		{
			if (SDL_SetColorKey(ptr, flag, key))
			{
				throw except::SDL2_Error();
			}
		}

		Uint32 map_rgb(const Color &color)
		{
			return SDL_MapRGB(ptr->format, color.r, color.g, color.b);
		}
