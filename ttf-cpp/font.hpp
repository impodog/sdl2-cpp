...
namespace sdl2::ttf
{
	...
	public:
		...
		[[nodiscard]] TTF_Font *operator->() const noexcept
		{
			return ptr;
		}
		...
		[[nodiscard]] Point size_text(const char *text) const
		{
			int w, h;
			if (TTF_SizeText(ptr, text, &w, &h))
			{
				throw except::SDL2_Error();
			}
			return {w, h};
		...
		[[nodiscard]] Point size_utf8(const char *text) const
		{
			int w, h;
			if (TTF_SizeUTF8(ptr, text, &w, &h))
			{
				throw except::SDL2_Error();
			}
			return {w, h};
		...
		[[nodiscard]] Point size_glyph(Uint16 ch) const
		{
			static char buf[sizeof(Uint16) + 1];

			::memcpy(buf, &ch, sizeof(Uint16));
			buf[sizeof(Uint16)] = '\0';

			return size_utf8(buf);
		}
		...
		render_blended_wrapped(const char *text, const Color &color, Uint32 wrap_length = 0) const
		{
		...
		[[nodiscard]] std::unique_ptr<Surface>
		render_utf8_blended_wrapped(const char *text, const Color &color, Uint32 wrap_length = 0) const
		{
